...
export const EXTRA_PATTERNS: Pattern[] = [
  ...
  /* ─── dynamic-workflow ─── */
  {
    ...
    nodes: [
      { id: 'user',       x: 20,  y: 240, w: 80,  h: 48, label: 'User', kind: 'user' },
      { id: 'claude',     x: 114, y: 180, w: 120, h: 46, label: 'Claude', sub: 'writes script', kind: 'accent' },
      { id: 'approve',    x: 114, y: 250, w: 120, h: 46, label: 'Approval', sub: 'phases · cost', kind: 'plain' },
      { id: 'script',     x: 250, y: 180, w: 130, h: 46, label: 'Workflow Script', sub: 'holds the phases', kind: 'accent' },
      { id: 'runtime',    x: 250, y: 250, w: 130, h: 46, label: 'Runtime', sub: 'executes · background', kind: 'bus' },
      { id: 'checkpoint', x: 258, y: 332, w: 84,  label: 'Checkpoint', sub: 'agent results', kind: 'store' },
      { id: 'find',       x: 400, y: 200, w: 120, h: 70, label: 'Find phase', sub: '~80 finders' },
      { id: 'verify',     x: 545, y: 192, w: 150, h: 86, label: 'Verify phase', sub: 'finding × votes' },
      { id: 'synth',      x: 715, y: 205, w: 112, h: 62, label: 'Synthesize', sub: '1 agent' },
      { id: 'final',      x: 848, y: 214, w: 48,  label: 'Report', kind: 'dark' },
    ],
    edges: {
      'u-c':  { from: 'user', to: 'claude', label: 'large task', curve: -14 },
      'u-a':  { from: 'user', to: 'approve', label: 'confirm', curve: 14 },
      'c-a':  { from: 'claude', to: 'approve', label: 'phase plan' },
      'c-s':  { from: 'claude', to: 'script', label: 'write JS' },
      'a-r':  { from: 'approve', to: 'runtime', label: 'allow run' },
      'r-sc': { from: 'runtime', to: 'script', label: 'executes' },
      'sc-f': { from: 'script', to: 'find', label: 'spawn phase', curve: -16 },
      'f-v':  { from: 'find', to: 'verify', label: 'findings stream' },
      'v-s':  { from: 'verify', to: 'synth', label: 'survivors' },
      's-rp': { from: 'synth', to: 'final', label: 'report' },
      'r-cp': { from: 'runtime', to: 'checkpoint', label: 'caches results' },
      'cp-r': { from: 'checkpoint', to: 'runtime', label: 'resume · relaunch', dashed: true, curve: -22 },
    },
    timeline: [
      {
        caption: '<b>User</b> hands Claude a task too large for one conversational pass.',
        captionZh: '<b>用户</b>把一个单轮对话难以稳定完成的大任务交给 Claude。',
        fire: ['u-c'],
        activate: ['user', 'claude'],
      },
      {
        caption: '<b>Claude</b> writes a workflow script whose plan is a sequence of named <b>phases</b>; the user confirms phases & cost at the approval gate.',
        captionZh: '<b>Claude</b>把计划写成 workflow 脚本——计划就是一串有名字的 <b>phase</b>；用户在审批门确认阶段与成本。',
        fire: ['c-s', 'c-a', 'u-a'],
        activate: ['user', 'claude', 'approve', 'script'],
      },
      {
        caption: 'The <b>runtime</b> executes the script in the background. The <b>script</b> — not Claude turn-by-turn — decides what runs next.',
        captionZh: '<b>Runtime</b> 在后台执行脚本。是<b>脚本</b>（而非 Claude 逐轮）决定下一步跑什么。',
        fire: ['a-r', 'r-sc'],
        activate: ['approve', 'runtime', 'script'],
      },
      {
        caption: '<b>Find phase</b>: the script fans out ~80 finder agents across shards, looping until no new findings turn up (loop-until-dry).',
        captionZh: '<b>Find phase</b>：脚本按分片扇出约 80 个 finder agent，循环到没有新发现为止（loop-until-dry）。',
        fire: ['sc-f'],
        activate: ['script', 'find'],
      },
      {
        caption: 'Findings <b>stream</b> into the Verify phase as each is produced — pipeline by default, not a gather-everything-then-verify barrier.',
        captionZh: '发现<b>边产出边流入</b> Verify phase——默认是 pipeline 流式，而非「全收齐再验证」的 barrier。',
        fire: ['f-v'],
        activate: ['find', 'verify'],
      },
      {
        caption: '<b>Verify phase</b>: each finding is adversarially checked by several votes. findings × votes makes this the <b>heaviest</b> fan-out (hundreds of agents).',
        captionZh: '<b>Verify phase</b>：每条发现由多票对抗校验。发现数 × 投票数让它成为<b>最重</b>的扇出（数百个 agent）。',
        fire: ['v-s'],
        activate: ['verify', 'synth'],
      },
      {
        caption: '<b>Synthesize phase</b>: one agent writes the report from the surviving findings; the runtime returns it to the session.',
        captionZh: '<b>Synthesize phase</b>：一个 agent 用通过校验的发现撰写报告；runtime 把它返回会话。',
        fire: ['s-rp'],
        activate: ['synth', 'final'],
      },
      {
        caption: 'The runtime caches each agent result (<b>Checkpoint</b>); a stopped run resumes only when <b>Claude relaunches</b> it with the runId, same session. Big tasks chain several workflows, Claude reviewing each before the next.',
        captionZh: 'Runtime 缓存每个 agent 结果（<b>Checkpoint</b>）；停掉的运行只有 <b>Claude 带 runId 重新发起</b>、且同一会话内才能恢复。大任务会链式跑多个 workflow，Claude 每次看完再决定下一步。',
        fire: ['r-cp', 'cp-r'],
        activate: ['runtime', 'checkpoint', 'claude'],
      },
    ],
    decorations: [
      { type: 'rect', x: 388, y: 168, w: 142, h: 162, label: 'FIND' },
      { type: 'rect', x: 535, y: 158, w: 172, h: 182, label: 'VERIFY' },
      { type: 'rect', x: 702, y: 168, w: 140, h: 162, label: 'SYNTHESIZE' },
    ],
    fit:
  ...
  return ctx.synthesize(checked.filter(x => x.verified))
    ...
    variants: [
      { label: 'Default', labelZh: '默认', sub: 'script → fan-out → verify', subZh: '脚本 → 扇出 → 验证', timeline: null },
      {
        ...
        timeline: [
          { caption: 'The script issues one barrier stage: every finder must finish before anything moves on.', captionZh: '脚本下达一个 barrier 阶段：所有 finder 必须全部完成才进入下一步。', activate: ['script'] },
          { caption: 'All ~80 finders start together in the Find phase.', captionZh: 'Find phase 里约 80 个 finder 同时启动。', fire: ['sc-f'], activate: ['script', 'find'] },
          { caption: 'Barrier: the Find phase blocks until the slowest finder returns — fast finders sit idle.', captionZh: 'Barrier：Find phase 阻塞到最慢的 finder 返回——快的 finder 空转等待。', activate: ['find'], duration: 1800 },
          { caption: 'Only after the whole batch completes do findings move to the Verify phase.', captionZh: '整批完成后，发现才整体进入 Verify phase。', fire: ['f-v'], activate: ['find', 'verify'] },
          { caption: 'Survivors pass to Synthesize, which writes the report.', captionZh: '通过校验的结论交给 Synthesize 撰写报告。', fire: ['v-s', 's-rp'], activate: ['verify', 'synth', 'final'] },
        ],
      },
      {
        label: 'Pipeline stream', labelZh: '流水线', sub: 'item by item', subZh: '逐 item 流动',
        timeline: [
          { caption: 'The script wires a streaming pipeline: Find → Verify → Synthesize, with no barrier between stages.', captionZh: '脚本接一条流式 pipeline：Find → Verify → Synthesize，stage 间没有 barrier。', activate: ['script', 'runtime'] },
          { caption: 'A finder produces finding A while later shards are still being searched.', captionZh: '一个 finder 产出 finding A 时，后续分片还在搜索中。', fire: ['sc-f'], activate: ['script', 'find'] },
          { caption: 'Finding A flows straight into Verify without waiting for the whole batch.', captionZh: 'finding A 不等整批，直接流入 Verify。', fire: ['f-v'], activate: ['find', 'verify'] },
          { caption: 'The runtime checkpoints partial progress as items keep moving.', captionZh: 'item 持续流动，runtime 随手 checkpoint 部分进度。', fire: ['r-cp'], activate: ['runtime', 'checkpoint'] },
          { caption: 'Synthesize begins from verified partials — wall-clock = the slowest single chain, not the sum of stages.', captionZh: 'Synthesize 从已验证的部分结果开始——总耗时 = 最慢一条链路，而非各 stage 之和。', fire: ['v-s', 's-rp'], activate: ['verify', 'synth', 'final'] },
        ],
      },
      {
        label: 'Adversarial review', labelZh: '对抗验证', sub: 'break weak claims', subZh: '反驳弱结论',
        timeline: [
          { caption: 'Finders produce independent findings from different slices of the task.', captionZh: 'Finder 从不同切片产出相互独立的发现。', fire: ['sc-f'], activate: ['script', 'find'] },
          { caption: 'Each finding is sent to a verifier that tries to disprove it (several votes).', captionZh: '每条发现交给 verifier 尝试反驳（多票）。', fire: ['f-v'], activate: ['find', 'verify'] },
          { caption: 'Weak or unsupported claims are filtered out before they reach synthesis.', captionZh: '薄弱或无依据的结论在进入综合前被过滤掉。', activate: ['verify'], duration: 1400 },
          { caption: 'Disputed areas loop back into the Find phase (loop-until-dry) for more evidence.', captionZh: '争议区域回到 Find phase（loop-until-dry）补证据。', fire: ['sc-f', 'f-v'], activate: ['script', 'find', 'verify'] },
          { caption: 'Converged, surviving claims are synthesized into one report.', captionZh: '收敛后通过校验的结论被综合成一份报告。', fire: ['v-s', 's-rp'], activate: ['verify', 'synth', 'final'] },
        ],
      },
    ],
  },
